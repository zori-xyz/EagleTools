...
async def _delete_job(db: AsyncSession, job_id: int) -> int:
    t = Job.__table__
    res = await db.execute(delete(t).where(t.c.id == job_id))
    await db.commit()
...
@router.get("/recent")
async def recent_list(db: AsyncSession = Depends(get_db)):
    # UI-контракт: { items: [...] }
    t = Job.__table__
    cols = list(t.c)

    q = select(*cols).select_from(t).order_by(t.c.id.desc()).limit(50)
...
async def recent_clear(payload: dict[str, Any] | None = None, db: AsyncSession = Depends(get_db)):
    # payload может содержать delete_files, пока игнорируем
    t = Job.__table__
    res = await db.execute(delete(t))
    await db.commit()
    return {"ok": True, "deleted": int(res.rowcount or 0)}


@router.post("/recent/delete")
async def recent_delete(payload: dict[str, Any], db: AsyncSession = Depends(get_db)):
    ...
    deleted = await _delete_job(db, jid)
    if deleted <= 0:
...
async def recent_delete_rest(job_id: int, db: AsyncSession = Depends(get_db)):
    deleted = await _delete_job(db, job_id)
    if deleted <= 0:
        raise HTTPException(status_code=404, detail="job_not_found")
